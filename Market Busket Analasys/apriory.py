...
df["month"].replace([ i for i in range (1, 12 + 1)], ["January", "Februari", "March", "April", "May", "June","July", "August", "September", "October", "November", "December"], inplace=True)

df["day"].replace([i for i in range(6 + 1)], ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday","Saturday", "Sunday"], inplace=True)

st.title("Market Basket Analisys Menggunakan Algoritma Apriori")
...
def user_input_features():
    ...
    weekday_weekend = st.selectbox('Weekday / Weekende', ['Weekday', 'Weekend'])
    month = st.select_slider('Month', ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"])
    day = st.select_slider('Day',["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"])
...
if type(data) != type ("No Result"):
    ...
    rule = association_rules(frequent_items, metric=metric, min_threshold= min_threshold)[["antecedents", "consequents", "support","confidence", "lift"]]
...
def return_item_df(item_antecedents):
    data = rule[["antecedents", "consequents"]].copy()
